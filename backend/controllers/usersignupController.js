...
try {
    ...
    const newUser = await buyerModel.create({name, email, password: hashedPassword, cpassword: hashedPassword , role})
...
try {
   const loggedInUser = await buyerModel.findOne({email})
   if(!loggedInUser){
...
//    const userPayload = { email }
   ...
   return res.status(400).send({status: 'error', msg: "error logging in"})
}
